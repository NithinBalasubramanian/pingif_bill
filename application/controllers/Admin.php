...
class Admin extends CI_Controller{
	...
	public function index()
		...
		public function Login($tablename)
        {
            ...
            $check = $this->Admin_model->table_column($tablename,'email',$email,'password',$password);
            if(count($check) > 0)
            {
	...
	public function List_customer()
	{
		...
		$output = '';
		$cus_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','1');
		if(count($cus_data)>0){
	   foreach($cus_data as $cus_row){ 
		   $output .= '<tr class="dark_mode">
			   ...
			   <td>
				<button class="btn btn-info btn-sm edit" data-url="'.base_url().'View/customer/edit_customer" data-id="'.$cus_row['id'].'">Edit</button>
	...
	public function List_del_customer()
	{
		...
		$tablename = $this->input->post('table');
		$output = '';
		$cus_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','0');
		if(count($cus_data)>0){
	   foreach($cus_data as $cus_row){ 
		   $output .= '<tr class="dark_mode">
			   <td>'.$i.'</td>
	...
	public function List_employee()
	{
		...
		$tablename = $this->input->post('table');
		$output = '';
		$emp_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','1');
		if(count($emp_data)>0){
	   foreach($emp_data as $emp_row){ 
		   $output .= '<tr class="dark_mode">
			   ...
			   <td>
				<button class="btn btn-info btn-sm edit" data-url="'.base_url().'View/customer/edit_customer" data-id="'.$emp_row['id'].'">Edit</button>
	...
	public function List_del_employee()
	{
		...
		$tablename = $this->input->post('table');
		$output = '';
		$emp_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','0');
		if(count($emp_data)>0){
	   foreach($emp_data as $emp_row){ 
		   $output .= '<tr class="dark_mode">
			   <td>'.$i.'</td>
	...
	public function List_supplier()
	{
		...
		$tablename = $this->input->post('table');
		$output = '';
		$supp_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','1');
	   ...
	   foreach($supp_data as $supp_row){ 
		   $output .= '<tr class="dark_mode">
			   ...
			   <td>
				<button class="btn btn-info btn-sm edit" data-url="'.base_url().'View/customer/edit_customer" data-id="'.$supp_row['id'].'">Edit</button>
	...
	public function List_del_supplier()
	{
		...
		$tablename = $this->input->post('table');
		$output = '';
		$supp_data = $this->Admin_model->table_column_desc_by_id($tablename,'status','0');
		if(count($supp_data)>0){
	   foreach($supp_data as $supp_row){ 
		   $output .= '<tr class="dark_mode">
			   <td>'.$i.'</td>
	...
	public function verify()
	{
		...
		$tablename=$this->input->post('table');
		$check = $this->Admin_model->table_column($tablename,'id',$id);
		foreach($check as $row){
			if($row['verify'] == '1'){
				$data['verify']='0';
			}else{
